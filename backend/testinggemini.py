...
# genai.configure(api_key=os.getenv("GEMINI_API_KEY", ""))  # set your key in env
genai.configure(api_key="AIzaSyCmRlApa14b-NWXDakx9iqlle9dzWjXHbc")  # set your key in env
...
def extract_toyota_filters(user_request):
    ...
    response = model.generate_content(
        prompt,
        generation_config={"response_mime_type": "application/json"}  # ensures pure JSON
    )

    # Print raw text (should now be clean JSON)
    print(response.text)
    print("-")
...
extract_toyota_filters("i want a cheat ass car")